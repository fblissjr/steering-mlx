...
class ControlledGemma3DecoderLayer(OriginalGemma3DecoderLayer):
    ...
    def _capture_if_needed(self, x: mx.array, point_name: str):
        if self._is_capturing and point_name in self._capture_targets:
            # mlx.core.array does not have a .copy() method.
            # Direct assignment is usually fine due to MLX's functional nature.
            # The array x is an output of a previous operation in the graph.
            # Storing it here captures that specific node in the graph.
            self._capture_activations_map[point_name] = x 
            logger.debug(f"Layer {self.layer_idx}: Captured activation for '{point_name}' with shape {x.shape}")
    ...
    def stop_capture(self) -> Dict[str, Optional[mx.array]]:
        ...
        # It's important to clear the stored activations after they are retrieved
        # to free up references if the graph is re-evaluated multiple times (e.g. in a loop for derivation)
        for cp in self._capture_targets: # Only clear what was targeted for this capture session
             self._capture_activations_map[cp] = None
